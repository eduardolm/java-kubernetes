...
run-db: stop-db rm-db
	docker run --name mysq -p 3306:3306 -e MYSQL_ROOT_PASSWORD=2377168 -e MYSQL_USER=root -e MYSQL_PASSWORD=2377168 -e MYSQL_DATABASE=k8s_java -d mysql:latest

run-app: stop-app rm-app
	docker run --name myapp -p 8080:8080 -d -e DATABASE_SERVER_NAME=mysql --link mysql:mysql java-k8s:latest
